...
- **A real bug** (e.g. a dependency changes its API, or Scholar changes its
  page layout) → an unexpected error on an attempt that reached Scholar
  directly (or through ScraperAPI) makes the script exit with an error, and
  the workflow run goes **red**, so you actually hear about the things worth
  fixing. Errors on the free-proxy retries are treated like a block, because
  a free proxy often returns a page that is not Scholar's.
