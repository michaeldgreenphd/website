...
├── manuscripts/                # Publication PDFs (add new papers here)
├── cv/                         # CV PDFs (add updated CVs here)
├── images/                     # Photos and image assets
├── docs/                       # Setup & upload guides
├── scripts/                    # Data pipeline (Scholar/Substack fetchers)
├── .github/workflows/          # Daily data-refresh automation
├── scholar_stats.json          # ┐
├── substack_posts.json         # │ Auto-generated by the daily workflow —
├── citations_over_time*.png    # ┘ do not edit by hand
└── README.md                   # This file
```

When adding a new manuscript or CV, upload the PDF into `manuscripts/` or
`cv/` and link it as `manuscripts/Your File.pdf` / `cv/Your File.pdf`.
See `docs/UPLOADING_FILES_GUIDE.md` for details.
